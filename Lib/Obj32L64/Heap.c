...
{
	__PUT(__VAL(Heap_ADR, typ), m->types, LONGINT);
	m->types = typ;
}
...
static LONGINT Heap_NewChunk (LONGINT blksz)
{
	...
	if (chnk != 0) {
		__PUT((Heap_ADR)(chnk + 8), chnk + (24 + blksz), LONGINT);
		__PUT((Heap_ADR)(chnk + 24), chnk + 32, LONGINT);
		__PUT((Heap_ADR)(chnk + 32), blksz, LONGINT);
		__PUT((Heap_ADR)(chnk + 40), -8, LONGINT);
		__PUT((Heap_ADR)(chnk + 48), Heap_bigBlocks, LONGINT);
...
static void Heap_ExtendHeap (LONGINT blksz)
{
	...
	if (chnk != 0) {
		if (chnk < Heap_heap) {
			__PUT(__VAL(Heap_ADR, chnk), Heap_heap, LONGINT);
			Heap_heap = chnk;
		} else {
			j = Heap_heap;
			...
			__PUT(__VAL(Heap_ADR, chnk), next, LONGINT);
			__PUT(__VAL(Heap_ADR, j), chnk, LONGINT);
		}
		if (next == 0) {
...
SYSTEM_PTR Heap_NEWREC (LONGINT tag)
{
	...
	if (i < 9) {
		...
		if (i != i0) {
			...
			__PUT((Heap_ADR)(end + 8), blksz, LONGINT);
			__PUT((Heap_ADR)(end + 16), -8, LONGINT);
			__PUT(__VAL(Heap_ADR, end), end + 8, LONGINT);
			__PUT((Heap_ADR)(adr + 8), restsize, LONGINT);
			__PUT((Heap_ADR)(adr + 24), Heap_freeList[di], LONGINT);
		...
		if (restsize > 288) {
			...
			if (prev == 0) {
				...
				__PUT((Heap_ADR)(prev + 24), next, LONGINT);
			}
			if (restsize > 0) {
	...
	while (i < end) {
		__PUT(__VAL(Heap_ADR, i), 0, LONGINT);
		__PUT((Heap_ADR)(i + 8), 0, LONGINT);
		__PUT((Heap_ADR)(i + 16), 0, LONGINT);
		__PUT((Heap_ADR)(i + 24), 0, LONGINT);
		i += 32;
	}
	__PUT((Heap_ADR)(adr + 24), 0, LONGINT);
	__PUT(__VAL(Heap_ADR, adr), tag, LONGINT);
	__PUT((Heap_ADR)(adr + 8), 0, LONGINT);
	__PUT((Heap_ADR)(adr + 16), 0, LONGINT);
...
SYSTEM_PTR Heap_NEWBLK (LONGINT size)
{
	...
	__PUT((Heap_ADR)(tag - 8), 0, LONGINT);
	__PUT(__VAL(Heap_ADR, tag), blksz, LONGINT);
	__PUT((Heap_ADR)(tag + 8), -8, LONGINT);
	__PUT((Heap_ADR)(__VAL(LONGINT, new) - 8), tag, LONGINT);
	Heap_Unlock();
	return new;
}
...
static void Heap_Mark (LONGINT q)
{
	...
	if (q != 0) {
		...
		if (!__ODD(tagbits)) {
			__PUT((Heap_ADR)(q - 8), tagbits + 1, LONGINT);
			p = 0;
			tag = tagbits + 8;
			for (;;) {
				__GET(__VAL(Heap_ADR, tag), offset, LONGINT);
				if (offset < 0) {
					__PUT((Heap_ADR)(q - 8), (tag + offset) + 1, LONGINT);
					if (p == 0) {
						break;
					}
					n = q;
					...
					__PUT(__VAL(Heap_ADR, fld), __VAL(SYSTEM_PTR, n), SYSTEM_PTR);
				} else {
					fld = q + offset;
					...
					if (n != 0) {
						...
						if (!__ODD(tagbits)) {
							__PUT((Heap_ADR)(n - 8), tagbits + 1, LONGINT);
							__PUT((Heap_ADR)(q - 8), tag + 1, LONGINT);
							__PUT(__VAL(Heap_ADR, fld), __VAL(SYSTEM_PTR, p), SYSTEM_PTR);
							p = q;
							q = n;
							tag = tagbits;
						}
	...
	while (chnk != 0) {
		...
		while (adr < end) {
			...
			if (__ODD(tag)) {
				if (freesize > 0) {
					start = adr - freesize;
					__PUT(__VAL(Heap_ADR, start), start + 8, LONGINT);
					__PUT((Heap_ADR)(start + 8), freesize, LONGINT);
					__PUT((Heap_ADR)(start + 16), -8, LONGINT);
					i = __ASHR(freesize, 5, LONGINT);
					freesize = 0;
					if (i < 9) {
						__PUT((Heap_ADR)(start + 24), Heap_freeList[i], LONGINT);
						Heap_freeList[i] = start;
					} else {
						__PUT((Heap_ADR)(start + 24), Heap_bigBlocks, LONGINT);
...
static void Heap_MarkStack (LONGINT n, LONGINT *cand, LONGINT cand__len)
{
	...
	if (n == 0) {
		...
		while (sp != stack0) {
			__GET(__VAL(Heap_ADR, sp), p, LONGINT);
...
void Heap_InitHeap (void)
{
	...
	__PUT(__VAL(Heap_ADR, Heap_heap), 0, LONGINT);
	Heap_allocated = 0;
	Heap_firstTry = 1;
	Heap_freeList[9] = 1;
	Heap_lockdepth = 0;
...
__TDESC(Heap_ModuleDesc__desc, 1, 2) = {__TDFLDS("ModuleDesc", 72), {0, 32, -24}};
__TDESC(Heap_CmdDesc__desc, 1, 1) = {__TDFLDS("CmdDesc", 32), {0, -16}};
__TDESC(Heap_FinDesc__desc, 1, 1) = {__TDFLDS("FinDesc", 24), {0, -16}};
__TDESC(Heap__1__desc, 1, 1) = {__TDFLDS("", 8), {4, -16}};
