			...
			if (adr == 0) {
				if (SYSTEM_firstTry) {
					...
					if (__ASHL((SYSTEM_heapsize - SYSTEM_allocated) - blksz, 2, LONGINT) < SYSTEM_heapsize) {
						SYSTEM_ExtendHeap(__ASHL(__DIV(SYSTEM_allocated + blksz, 96), 7, LONGINT) - SYSTEM_heapsize);
					}
					SYSTEM_firstTry = 0;
...
__TDESC(SYSTEM_ModuleDesc__desc, 1, 2) = {__TDFLDS("ModuleDesc", 72), {0, 32, -24}};
__TDESC(SYSTEM_CmdDesc__desc, 1, 1) = {__TDFLDS("CmdDesc", 32), {0, -16}};
__TDESC(SYSTEM_FinDesc__desc, 1, 1) = {__TDFLDS("FinDesc", 24), {0, -16}};
__TDESC(SYSTEM__1__desc, 1, 1) = {__TDFLDS("", 8), {4, -16}};
