...
static void Files_Flush (Files_Buffer buf)
{
	...
	if (buf->chg) {
		...
		error = Platform_Write(f->fd, (INTEGER)buf->data, buf->size);
		if (error != 0) {
...
void Files_ReadBytes (Files_Rider *r, LONGINT *r__typ, BYTE *x, LONGINT x__len, LONGINT n)
{
	...
	while (n > 0) {
		...
		__MOVE((INTEGER)buf->data + (INTEGER)offset, (INTEGER)x + (INTEGER)xpos, min);
		offset += min;
		(*r).offset = offset;
		xpos += min;
		n -= min;
...
void Files_WriteBytes (Files_Rider *r, LONGINT *r__typ, BYTE *x, LONGINT x__len, LONGINT n)
{
	...
	while (n > 0) {
		...
		__MOVE((INTEGER)x + (INTEGER)xpos, (INTEGER)buf->data + (INTEGER)offset, min);
		offset += min;
		(*r).offset = offset;
		if (offset > buf->size) {
	...
	if (error == 0) {
		...
		if (!Platform_DifferentFilesystems(error)) {
			*res = error;
			...
			error = Platform_Read(fdold, (INTEGER)buf, 4096, &n);
			while (n > 0) {
				error = Platform_Write(fdnew, (INTEGER)buf, n);
				if (error != 0) {
...
__TDESC(Files_Handle__desc, 1, 4) = {__TDFLDS("Handle", 260), {236, 240, 244, 248, -20}};
