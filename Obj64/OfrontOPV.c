			...
			switch (subclass) {
				...
				case 14: 
					if (__IN(l->typ->form, 0x8400)) {
						OfrontOPM_WriteString((CHAR*)"__STRCMP(", 10);
						OfrontOPV_expr(l, -1);
						OfrontOPM_WriteString((CHAR*)", ", 3);
						OfrontOPV_expr(r, -1);
						OfrontOPM_Write(')');
...
static void OfrontOPV_IfStat (OfrontOPT_Node n, BOOLEAN withtrap, OfrontOPT_Object outerProc)
{
	...
	if (withtrap) {
		OfrontOPM_WriteString((CHAR*)"__WITHCHK", 10);
		OfrontOPC_EndStat();
	} else if (n->right != NIL) {
		OfrontOPC_BegBlk();
...
static void OfrontOPV_CaseStat (OfrontOPT_Node n, OfrontOPT_Object outerProc)
{
	...
	if (n->right->conval->setval != 0x0) {
		...
		OfrontOPM_WriteString((CHAR*)"__CASECHK", 10);
	}
	OfrontOPC_EndStat();
	OfrontOPC_EndBlk();
}
...
/*----------------------------------------------------------------------------*/
__TDESC(OfrontOPV_ExitInfo__desc, 1, 0) = {__TDFLDS("ExitInfo", 8), {-8}};
