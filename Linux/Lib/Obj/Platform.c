...
#define Platform_allocate(size)	(LONGINT)(SYSTEM_ADR)((void*)malloc((size_t)size))
...
#define Platform_free(address)	free((void*)(SYSTEM_ADR)address)
...
#define Platform_readfile(fd, p, l)	read(fd, (void*)(SYSTEM_ADR)(p), l)
...
#define Platform_writefile(fd, p, l)	write(fd, (void*)(SYSTEM_ADR)(p), l)
...
static void Platform_YMDHMStoClock (LONGINT ye, LONGINT mo, LONGINT da, LONGINT ho, LONGINT mi, LONGINT se, LONGINT *t, LONGINT *d)
{
	*d = (__ASHL(__MOD(ye, 100), 9, LONGINT) + __ASHL(mo + 1, 5, LONGINT)) + da;
	*t = (__ASHL(ho, 12, LONGINT) + __ASHL(mi, 6, LONGINT)) + se;
}
...
{
	*n = Platform_readfile(h, (LONGINT)b, b__len);
	if (*n < 0) {
		*n = 0;
		return Platform_err();
	} else {
