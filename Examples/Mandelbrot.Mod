...
PROCEDURE Go(sx, sy: INTEGER; x, y: SHORTREAL);
CONST iter = 768;
VAR col, i: INTEGER;
  re, im, re2: SHORTREAL;
BEGIN re := x; im := y; i := 0;
...
PROCEDURE Do;
VAR x, y: INTEGER;
  x0, y0, x1, y1: SHORTREAL;
BEGIN G.Settings(640, 480, {G.spread});
  s := G.Init();
  IF s = NIL THEN Out.String("Couldn't init graphic"); Out.Ln; HALT(1) END;
  black := G.MakeCol(0, 0, 0);
  y0 := 1; y1 := -1;
  x1 := 2 * SHORT(s.w / s.h / 3);
  x0 := -x1 * 2;
  FOR y := 0 TO s.h - 1 DO
    FOR x := 0 TO s.w - 1 DO
      Go(x, y, x0 + SHORT(x / (s.w - 1)) * (x1 - x0),
               y0 + SHORT(y / (s.h - 1)) * (y1 - y0))
    END
  END;
