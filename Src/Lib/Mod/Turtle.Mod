...
VAR
  screen: G.Bitmap; x-, y-, angle: SHORTREAL; color: Color;
...
PROCEDURE Forward* (distance: SHORTREAL);
VAR
  newx, newy: SHORTREAL;
BEGIN
...
PROCEDURE Back* (distance: SHORTREAL);
BEGIN
  Forward(-distance);
END Back;

PROCEDURE Left* (a: SHORTREAL);
BEGIN
  angle := angle - a;
END Left;

PROCEDURE Right* (a: SHORTREAL);
BEGIN
  Left(-a);
END Right;
...
PROCEDURE Triangle (s: SHORTREAL);
CONST
  Bd = 110;
BEGIN
